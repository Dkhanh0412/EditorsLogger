...
def get_database_path():
    """Get the correct database path for both development and bundled environments"""
    if getattr(sys, 'frozen', False):
        # Running as a bundled executable
        app_dir = os.path.dirname(sys.executable)
        db_path = os.path.join(app_dir, 'projects.db')
    else:
        # Running as a script
        script_dir = os.path.dirname(os.path.abspath(__file__))
        db_path = os.path.join(script_dir, 'projects.db')
    return db_path
...
class TranslationManager:
    ...
    def _load_translations(self, json_path="translations.json"):
        """Load translations from JSON file - works in both dev and bundled environments"""
        try:
            ...
            if getattr(sys, 'frozen', False):
                ...
                # Method 1: Check sys._MEIPASS (for --onefile)
                if hasattr(sys, '_MEIPASS'):
                    meipass_path = os.path.join(sys._MEIPASS, json_path)
                    print(f"  Trying _MEIPASS: {meipass_path}")
                    if os.path.exists(meipass_path):
                        with open(meipass_path, 'r', encoding='utf-8') as f:
                            print(f"  ✓ Loaded from _MEIPASS")
                            return json.load(f)
                
                # Method 2: Check executable directory (for --onedir)
                exe_dir = os.path.dirname(sys.executable)
                exe_path = os.path.join(exe_dir, json_path)
                print(f"  Trying exe dir: {exe_path}")
                if os.path.exists(exe_path):
                    with open(exe_path, 'r', encoding='utf-8') as f:
                        print(f"  ✓ Loaded from exe dir")
                        return json.load(f)
                
                # Method 3: For macOS .app bundles
                if sys.platform == 'darwin' and '.app' in sys.executable:
                    # Navigate to .app/Contents/Resources
                    app_dir = sys.executable
                    while 'Contents' in app_dir:
                        app_dir = os.path.dirname(app_dir)
                        if app_dir.endswith('.app'):
                            break
                    
                    if app_dir.endswith('.app'):
                        resources_path = os.path.join(app_dir, 'Contents', 'Resources', json_path)
                        print(f"  Trying .app Resources: {resources_path}")
                        if os.path.exists(resources_path):
                            with open(resources_path, 'r', encoding='utf-8') as f:
                                print(f"  ✓ Loaded from .app Resources")
                                return json.load(f)
                
                print(f"  ⚠ {json_path} not found in any bundle location")
                
            else:
                # Development mode
                script_dir = os.path.dirname(os.path.abspath(__file__))
                dev_path = os.path.join(script_dir, json_path)
                print(f"  Trying dev path: {dev_path}")
                
                if os.path.exists(dev_path):
                    with open(dev_path, 'r', encoding='utf-8') as f:
                        print(f"  ✓ Loaded from dev path")
                        return json.load(f)
                else:
                    print(f"  ⚠ {json_path} not found in dev")
...
class ProjectManager:
    ...
    def __init__(self, db_path=None):
        # SIMPLE FIX: ALWAYS use Documents folder for both dev and bundled
        # This ensures both versions use the same database file
    ...
    def init_db(self):
        ...
        # Check how many projects exist after init
        cursor.execute('SELECT COUNT(*) FROM projects')
        count = cursor.fetchone()[0]
        print(f"   → Database initialized, contains {count} projects")
        
        conn.close()
    
    # Keep all your other methods (get_projects, create_project, etc.) unchanged
    def get_projects(self):
        """Get all projects"""
        print(f"📋 Getting projects from: {self.db_path}")
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute('SELECT name, modified_date FROM projects ORDER BY modified_date DESC')
        projects = cursor.fetchall()
...
class EditorsLogProV5(QMainWindow):
    ...
    def show_home_screen(self):
        ...
        home.language_changed.connect(self.on_language_changed)
        self.setCentralWidget(home)
        self.apply_theme()

    def on_language_changed(self):
        """Handle language change from home screen"""
        # If we're in editor mode, refresh the UI
        if hasattr(self, 'table'):  # Check if editor UI is loaded
            # Simple refresh - update a few key elements
            if hasattr(self, 'back_btn'):
                self.back_btn.setText(self.translator.t("← Back to Projects"))
            if hasattr(self, 'add_take_btn'):
                self.add_take_btn.setText(self.translator.t("➕ Add New Take"))
    
    def refresh_editor_ui(self):
        """Quick refresh of editor UI"""
        # Update window title
        self.setWindowTitle(f"Editor's Logger - v1.0.0")
        ...
        if hasattr(self, 'save_btn'):
            self.save_btn.setText(self.translator.t("💾 SAVE ANNOTATIONS"))
        
        # Update table headers
        if hasattr(self, 'table'):
            headers = [
                self.translator.t("SHOT"),
                self.translator.t("TK"),
                self.translator.t("FILE"),
                self.translator.t("SCORE"),
                self.translator.t("STATUS")
            ]
    ...
    def init_ui(self):
        ...
        left_panel.addWidget(QLabel(self.translator.t("LISTS:")))
